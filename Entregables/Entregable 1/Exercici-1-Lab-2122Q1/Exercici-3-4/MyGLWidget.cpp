...
//  glViewport (0, 0, ample, alt);
  ...
  for (int i = 0; i < NUM_ARBRES; i++) {
    ...
    glUniform3fv(colorArbreLoc, 1, &colorArbre[i][0]);

    modelTransformArbre(posicioArbre[i], midaArbre[i]);

    glUniform1i(unifocusLoc, 1);
    // Pintem tronc
    glBindVertexArray(VAOTronc);
    glDrawArrays(GL_TRIANGLES, 0, 6);

    glUniform1i(unifocusLoc, 0);
    // Pintem copa
...
{
  glm::mat4 TG (1.0);
  TG = glm::translate(TG, glm::vec3(posicio[0], posicio[1] + mida/2, posicio[2]));
  TG = glm::scale(TG, glm::vec3(mida/midaModel, mida/midaModel, 0.0));
...
{
  glUniform3fv(colorTroncLoc, 1, &colorTronc[0]);
...
void MyGLWidget::creaBuffersCopa ()
{
  ...
  if (square) {
    ...
    Vertices[4] = glm::vec3(-0.5, -0.2, 0);
    Vertices[5] = glm::vec3( 0.5, 0.8, 0);
  } else {
    Vertices[0] = glm::vec3( 0, 0.8, 0);
    Vertices[1] = glm::vec3(-0.4, -0.4, 0);
    Vertices[2] = glm::vec3( 0.4, -0.4, 0);
    Vertices[3] = glm::vec3( 0.4, -0.4, 0);
...
void MyGLWidget::carregaShaders()
{
  ...
  unifocusLoc = glGetUniformLocation (program->programId (), "unifocus");
  colorTroncLoc = glGetUniformLocation (program->programId (), "colorTronc");
  colorArbreLoc = glGetUniformLocation (program->programId (), "colorArbre");
